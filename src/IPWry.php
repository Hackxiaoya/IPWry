...
class IPWry
{
    ...
    public function __construct($file)
    {
        if (!file_exists($file) or !is_readable($file)) {
            throw new IPWryException("{$file} does not exist, or is not readable");
        }
        $this->file = $file;
     ...
     * @param string $ip
     * @return array
     * @throws IPWryException
     */
    public function query($ip)
    {
        $ip_split = explode('.', $ip);
        if (count($ip_split) !== 4) {
            throw new IPWryException("{$ip} is not a valid ip address");
        }
    ...
    protected function readRecord($offset)
    {
        ...
        foreach($this->provinces as $v) {
            if(strpos($record['country'],$v) === 0) {
                ...
                $isMatched = preg_match_all('/(.*?)市/', $record['city'], $matches);
                if ($isMatched) {
                    $record['city'] = $matches[0][0];
                }
                break;
            }
        }
